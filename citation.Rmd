...
I show how to inclcitation of `R` packages and/or external sources of literature within `Rmarkdown` file in `R` [@R-base]. Some of the packages include `tidyverse` version `r packageVersion('tidyverse')` [@R-tidyverse]. On the other hand, I also used external citations which are not necessary packages [_sensu_ @naimi2016sdm, pp.3; @naimi2014positional]. On the same note, Naimi [-@naimi2016sdm] suggested that predictor variables should be scaled to enable interpretation of variable importance values. @soberon2004biodiversity [pp. 25] had shown similar patterns earlier.
